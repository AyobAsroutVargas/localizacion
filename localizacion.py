...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  while(i < radio):
    ...
    fila_imagen = []
    while(j < radio):
      robot_aux.set(centro[0] + i, centro[1] + j, ideal.pose()[-1])
      error = real.measurement_prob(robot_aux.sense(objetivos), objetivos)
      fila_imagen.append(error)
    ...
    i+=incremento
    imagen.append(fila_imagen)

  ideal.set(mejor_pos[0], mejor_pos[1], ideal.pose()[-1])
...
EPSILON2 = .3              # Umbral de distancia para localizacion
...
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    ...
    diff1 = np.array(real.sense(objetivos))
    diff2 = np.array(ideal.sense(objetivos))
    difference = diff1 - diff2
    #print(difference)
    if(any(abs(diff) > EPSILON2 for diff in difference)):
    #if (distancia([real.x, real.y], [ideal.x, ideal.y]) > EPSILON2):
      #Copiado del mostrar
      objT   = np.array(objetivos).T.tolist()
      ...
      bordes = [min(trayT[0]+objT[0]+ideT[0]),max(trayT[0]+objT[0]+ideT[0]),
                min(trayT[1]+objT[1]+ideT[1]),max(trayT[1]+objT[1]+ideT[1])]
      centro = [(bordes[0]+bordes[1])/2.,(bordes[2]+bordes[3])/2.]
      radio  = max(bordes[1]-bordes[0],bordes[3]-bordes[2])*.5

      # print("centro " + ' '.join(map(str,centro)))
      # print("radio " + str(radio))
      # print(real.pose())

      localizacion(objetivos, real, ideal, centro, radio)
